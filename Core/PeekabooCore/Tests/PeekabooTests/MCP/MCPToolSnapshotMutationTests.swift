...
struct MCPToolSnapshotMutationTests {
    ...
    func `Ordinary mutation completion failure warns without inviting a retry`(toolName: String) async throws {
        ...
        let bridgedText = try #require(bridgedObject["result"] as? String)
        #expect(bridgedText.contains("Do not retry this mutation"))
        #expect(bridgedObject["text"] == nil)
        #expect(bridgedObject["content"] == nil)
        let bridgedMetadata = try #require(bridgedObject["meta"] as? [String: Any])
        let invalidation = try #require(bridgedMetadata["snapshot_invalidation"] as? [String: Any])
        #expect(invalidation["status"] as? String == "pending_retry")
        #expect(invalidation["tool_executed"] as? Bool == true)
        #expect(invalidation["retry_tool"] as? Bool == false)
